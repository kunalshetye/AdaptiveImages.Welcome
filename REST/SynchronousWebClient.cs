...
namespace AdaptiveImages.Welcome.REST
{
    public class SynchronousWebClient
    {
        ...
        public SynchronousWebClient(ILogger logger)
        {
            _logger = logger;
        }
        ...
        public T Get<T>(string url) where T : class, new()
        {
            T data = null;
            var uri = new Uri(url);
            var cacheKey = url.Replace($"{uri.Scheme}://{uri.Host}","");
            if (HasAccessToken)
            {
                var httpClient = new HttpClient();
                httpClient.DefaultRequestHeaders.Add("Authorization", $"Bearer {accessToken}");
                var responseJSON = httpClient.GetStringAsync(url).GetAwaiter().GetResult();
